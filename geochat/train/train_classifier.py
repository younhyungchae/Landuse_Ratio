...
from geochat.model.classifier import GeoChatLlamaForLMClassifier
...
def preprocess_classification(
    ...
    for i, source in enumerate(sources):
        conv.messages = []
        role = source[0]['from']
        conv.append_message(role, source[0]["value"])
        conversations.append(conv.get_prompt())
    ...
    target_ids = [[tokenizer(label, add_special_tokens=False)['input_ids'][0] for label in source[0]['label'].keys()] for source in sources]
    targets = torch.tensor([list(source[0]['label'].values()) for source in sources])
    assert len(list(set(target_ids[0]))) == len(target_ids[0])
    return dict(
        input_ids=input_ids,
...
def preprocess(
    ...
    return dict(input_ids=input_ids, labels=targets)
    '''
    return preprocess_classification(sources, tokenizer, has_image)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(i, int):
            data_dict = dict(input_ids=data_dict["input_ids"][0],
                             ...
                             target_pos=data_dict['target_pos'][0],
                             target_labels=data_dict["target_labels"][0])
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, target_labels, target_ids, target_pos = tuple([instance[key] for instance in instances]
                                  for key in ("input_ids", "target_labels", "target_ids", "target_pos"))
...
def train():
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
            ...
            model = GeoChatLlamaForLMClassifier.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                **bnb_model_from_pretrained_args
            )
